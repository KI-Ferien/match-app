# Create the complete, production-ready match.js code

code_content = """function getFallbackData(signs) {
  return {
...
Antworte AUSSCHLIESSLICH als JSON-Objekt ohne Markdown, ohne Präambel, ohne Erklärung:
{
  ...
  "cta_text": "Ferien Erlebnisse buchen"
    ...
    const getCityForDestination = (destName) => {
  ...
  "cta_text": "Ferien Erlebnisse buchen"
      ...
      if (correctionRes.ok) {
        ...
        try {
          ...
          correctedParsed.welcome_pickups_city = getCityForDestination(correctedParsed.destination);
